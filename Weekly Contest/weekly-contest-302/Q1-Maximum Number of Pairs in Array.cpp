...
public:
    vector<int> numberOfPairs(vector<int>& nums) {
        ...
        int left = 0;
            ...
            for(auto it: nums){ //iterate in the array and store the elements with their frequency in the map
                mp[it]++;
            }
            
            for(auto it: mp){// iterating in the map
                pairs += it.second/2; // calculating pairs
                it.second %=2; // modifing the freq of element after removing the pairs
                left += it.second; // adding the new frequency in the left
            }
